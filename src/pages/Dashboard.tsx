...
import { CalendarClock, PackageSearch, Search, ShoppingCart, WalletCards } from 'lucide-react';
...
export function Dashboard() {
  ...
  return <div className="dashboard-home min-w-0 space-y-4">
    ...
    <section className="grid min-w-0 gap-3 md:grid-cols-[minmax(260px,1.3fr)_minmax(0,1fr)_minmax(0,1fr)]">
      ...
      {cardsLoading ? <div className="col-span-2 glass-card flex min-h-[86px] items-center justify-center"><LoadingSpinner/></div> : <><MetricCard label="Today's Sales" value={(cards?.todaySales ?? 0).toLocaleString()} icon={WalletCards}/><MetricCard label="Today's Revenue" value={formatCurrency(cards?.todayRevenue ?? 0)} icon={ShoppingCart}/></>}
    </section>
