...
export interface CashupRow{session_id:string;cashup_number:string;user_id:string;cashier_name:string;opening_time:string;closing_time:string|null;opening_cash:number;total_invoices:number;items_sold:number;gross_sales:number;discounts:number;total_sales:number;cash_sales:number;card_sales:number;bank_sales:number;credit_sales:number;other_sales:number;cash_refunds:number;cash_expenses:number;bank_deposits:number;expected_cash:number;counted_cash:number|null;difference:number|null;status:'open'|'closed';difference_status:'open'|'balanced'|'short'|'over';notes:string|null}
export interface CashupSummary{sessions:number;total_sales:number;cash_sales:number;card_sales:number;bank_sales:number;credit_sales:number;cash_expenses:number;bank_deposits:number;expected_cash:number;counted_cash:number;cash_difference:number}
