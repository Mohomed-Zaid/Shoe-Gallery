...
export function ProductVariantSelector({ product, cartQuantities, lowStockLimit, keepOpen, onKeepOpenChange, onAdd, onClose, onViewCart }: Props) {
  ...
  return <Modal title="Select Product Variant" onClose={onClose} size="xl">
    <div className="space-y-5">
      <header className="flex flex-wrap items-center gap-4 rounded-2xl border border-white/10 bg-white/[.04] p-4">
        <div className="flex h-20 w-20 shrink-0 items-center justify-center overflow-hidden rounded-xl bg-dashboard-accent/15">
      ...
      {!variants.length ? <div className="rounded-xl border border-amber-400/20 bg-amber-400/10 p-5 text-amber-200">This product has no active variants.</div> : <>
        ...
        <div className="hidden overflow-x-auto md:block">
          <table className="w-full min-w-[700px] border-separate border-spacing-2"><thead><tr><th className="sticky left-0 z-10 bg-[#071b15] p-2 text-left text-sm text-dashboard-text-sub">Size</th>{colours.map((colour) => <th key={colour} className="sticky top-0 bg-[#071b15] p-2 text-center text-sm text-dashboard-text-label">{colour}</th>)}</tr></thead>
            <tbody>{sizes.map((size, row) => <tr key={size}><th className="sticky left-0 z-10 bg-[#071b15] p-2 text-left text-lg text-dashboard-text-primary">{size}</th>{colours.map((colour, column) => {
      ...
      <footer className="sticky bottom-0 z-20 -mx-4 flex flex-wrap justify-end gap-2 border-t border-white/10 bg-[#061711]/95 px-4 py-3 backdrop-blur sm:-mx-5 sm:px-5"><Button variant="outline" onClick={onViewCart}>View Cart</Button><Button variant="secondary" onClick={onClose}>Close</Button><Button onClick={onClose}>Done</Button></footer>
    </div>
  </Modal>;
}
