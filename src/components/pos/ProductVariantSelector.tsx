  ...
  return <Modal title="Select Product Variant" onClose={onClose} size="xl" respectSidebar>
    <div className="space-y-4">
      <header className="flex flex-wrap items-center gap-3 rounded-2xl border border-white/10 bg-white/[.04] p-3 sm:p-4">
        ...
        <div className="min-w-0 flex-1"><p className="text-xs font-semibold uppercase tracking-wider text-sky-300">Article Number: {product.item_article || product.item_number || product.code}</p><h2 className="text-xl font-bold text-dashboard-text-primary">{product.name}</h2><p className="text-sm text-dashboard-text-sub">{product.brand?.name || 'Unbranded'} · {product.category?.name || 'Uncategorized'}</p></div>
