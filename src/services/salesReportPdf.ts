...
export function downloadSimpleSalesReportPdf(
  ...
  autoTable(doc, {
    ...
    theme: 'grid',
    head: [['Total Sales', 'Card Fees (2.75%)', 'Invoices', 'Quantity', 'Received', 'Outstanding']],
    body: [[money(summary.total_sales), money(summary.total_card_payment_fees), summary.total_invoices, summary.total_quantity, money(summary.total_received), money(summary.total_outstanding)]],
  ...
  autoTable(doc, {
    ...
    theme: 'grid',
    head: [['Invoice', 'Date', 'Customer', 'Qty', 'Selling Price', 'Card Fee (2.75%)', 'Total', 'Paid', 'Balance', 'Payment', 'Status']],
