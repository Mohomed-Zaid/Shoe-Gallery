...
export function downloadSimpleSalesReportPdf(
  ...
  autoTable(doc, {
    ...
    theme: 'grid',
    head: [['Total Sales', 'Gross Card Amount', 'Card Fee (2.75%)', 'Net Card Amount', 'Invoices', 'Quantity', 'Received', 'Outstanding']],
    body: [[money(summary.total_sales), money(summary.total_gross_card_amount), money(-summary.total_card_processing_fees), money(summary.total_net_card_amount), summary.total_invoices, summary.total_quantity, money(summary.total_received), money(summary.total_outstanding)]],
  ...
  autoTable(doc, {
    ...
    theme: 'grid',
    head: [['Invoice', 'Date', 'Customer', 'Qty', 'Selling Price', 'Gross Card', 'Card Fee (2.75%)', 'Net Card', 'Total', 'Paid', 'Balance', 'Payment', 'Status']],
    body: data.rows.map((row) => [
      ...
      money(row.gross_card_amount),
      money(-row.card_processing_fee),
      money(row.net_card_amount),
      money(row.total),
